...
class Character:
    ...
    def update(self):
        ...
        if self.condition == 0 or self.condition == 1 or self.condition == 4:
            # 1. 충돌하면 1을 더한다.
            for i in range(tiles_grass1_count):
                if collipseCheck(self.frameX, self.frameY, self.x, self.y,
                                tiles_grass1[i].Width, tiles_grass1[i].Height,
                                tiles_grass1[i].startXPos, tiles_grass1[i].startYPos):
                    self.isOnGround += 1
...
#=== 지형
# 타일
tiles_grass1_count = 10
tiles_grass1 = [Tile() for i in range(tiles_grass1_count)]
tiles_grass1_startX, tiles_grass1_startY = 300, 50
for i in range(tiles_grass1_count):
    tiles_grass1[i].startXPos = tiles_grass1_startX
    tiles_grass1[i].startYPos = tiles_grass1_startY * (10 - tiles_grass1_count)
